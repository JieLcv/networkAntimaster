...
{
    //1. 对扑克牌进行分类:1张的，2张的，3张的， 4张的各有多少种
    //2. 对牌型进行分类
    classify(cards);
    judgeCardType();
}
...
{
    //我的牌型未知
    if(m_type == Hand_Unknown)return false;
    //对方放弃出牌
    if(other.m_type == Hand_Pass)return true;
    //我是王炸
    if(m_type == Hand_Bomb_Jokers)return true;
    //我是炸弹
    if(m_type == Hand_Bomb && other.m_type >= Hand_Single && other.m_type <= Hand_Seq_Single) return true;
    //双方牌型一致
    if(m_type == other.m_type)
    {
        if(m_type == Hand_Seq_Pair || m_type == Hand_Seq_Single)return m_pt > other.m_pt && m_extra == other.m_extra;
        else return m_pt > other.m_pt;
    }
    return false;
}
